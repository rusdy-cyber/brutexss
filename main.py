...
def brutexss():
    ...
    def again():
        inp = input("[?] [E]xit or launch [A]gain? (e/a)").lower()
        if inp == 'a':
            brutexss()
        elif inp == 'e':
            exit()
    ...
    def GET():
        try:
            ...
            if 'https://' in site:
                ...
                site = "http://" + site
            finalurl = urllib.parse.urlparse(site)
            urldata = urllib.parse.parse_qsl(finalurl.query)
            ...
            paraname = []
            paravalue = []
            wordlist = input("[?] Enter location of Wordlist (Press Enter to use default wordlist.txt)\n[?] > ")
            if len(wordlist) == 0:
            ...
            o = urllib.parse.urlparse(site)
            parameters = urllib.parse.parse_qs(o.query, keep_blank_values=True)
            path = urllib.parse.urlparse(site).scheme + "://" + urllib.parse.urlparse(site).netloc + urllib.parse.urlparse(site).path
            ...
            for pn, pv in zip(paraname, paravalue):  # Scanning the parameter.
                ...
                fpar.append(str(pn))
                for x in payloads:  #
                    validate = x.translate(str.maketrans('', '', whitespace))
                    if validate == "":
                        progress = progress + 1
                    else:
                        ...
                        sourcecode = page.read().decode('utf-8')
                        if x in sourcecode:
    ...
    def POST():
        try:
            try:
                ...
                path = urllib.parse.urlparse(site).scheme + "://" + urllib.parse.urlparse(site).netloc + urllib.parse.urlparse(site).path
                url = site
                param = str(input("[?] Enter post data: > "))
                wordlist = input("[?] Enter location of Wordlist (Press Enter to use default wordlist.txt)\n[?] > ")
                if len(wordlist) == 0:
                    wordlist = 'wordlist.txt'
                ...
                finalurl = urllib.parse.urlparse(params)
                urldata = urllib.parse.parse_qsl(finalurl.query)
                o = urllib.parse.urlparse(params)
                parameters = urllib.parse.parse_qs(o.query, keep_blank_values=True)
                paraname = []
                paravalue = []
                for para in parameters:  # Arranging parameters and values.
                ...
                for pn, pv in zip(paraname, paravalue):  # Scanning the parameter.
                    ...
                    fpar.append(str(pn))
                    for i in payloads:
                        validate = i.translate(str.maketrans('', '', whitespace))
                        if validate == "":
                            progress = progress + 1
                        else:
                            progress = progress + 1
                            ...
                            data = urllib.parse.urlencode(dict(zip(pname1, payload1))).encode('utf-8')
                            r = br.open(path, data)
                            sourcecode = r.read().decode('utf-8')
                            pname1 = []
                            payload1 = []
                            if i in sourcecode:
    ...
    try:
        methodselect = input("[?] Select method: [G]ET or [P]OST (G/P): ").lower()
        if methodselect == 'g':
            GET()
        elif methodselect == 'p':
            POST()
